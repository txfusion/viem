...
import { getL2TokenAddress } from '~viem/zksync/bridge-utils/l2TokenAddress.js'
...
test('DepositETHOnNonETHBasedChain', async () => {
  ...
  const bToken = await getBaseTokenL1Address(clientL1)

  const bridges = await clientL2.getDefaultBridgeAddresses()
  const l2EthAddress = await getL2TokenAddress(clientL2, {
    token: ethAddressInContracts,
    sharedL2: bridges.sharedL2,
    baseTokenAddress: bToken,
  })
